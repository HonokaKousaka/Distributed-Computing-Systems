...
public class Verifier {
     ...
     * An error is thrown if not passed.
     *
     * @param output a string of (Key, [Value List]) pairs need to be checked
     * @param answer a string of (Key, [Value List]) pairs as an answer
     * @param parser the parser of the output and the answer
     */
    public static void verifyKV(String output, String answer, KVListParser parser) throws Exception {
      verifyKV(output, answer, parser, parser);
    }
     ...
     * @param answerParser the parser of the answer
     */
    public static void verifyKV(String output, String answer, KVListParser outputParser, KVListParser answerParser)
        throws Exception {
  ...
  /**
   * Parse the output and the answer with the default parser; verify if the output is as same as the answer (in order).
   * An error is thrown if not passed.
   *
   * @param output a string of lists need to be checked
   * @param answer a string of lists as an answer
   */
  public static void verifyListInOrder(String output, String answer) {
    ListParser outputParser = new ListParser();
    ListParser answerParser = new ListParser();
    verifyListInOrder(output, answer, outputParser, answerParser);
  }
    ...
    public static void verifyList(String output, String answer, ListParser outputParser, ListParser answerParser) {
        ...
        assertEquals(answerList.size(),outputList.size());
  ...
  /**
   * Parse the output and the answer with the parser; verify if the output is as same as the answer (in order).
   * An error is thrown if not passed.
   *
   * @param output       a string of lists need to be checked
   * @param answer       a string of lists as an answer
   ...
   * @param answerParser the parser of the answer
   */
  public static void verifyListInOrder(String output, String answer, ListParser outputParser, ListParser answerParser) {
    List outputList = outputParser.parse(output);
    List answerList = answerParser.parse(answer);
    ...
    for(int i=0;i<answerList.size();i++){
      if(!outputList.get(i).equals(answerList.get(i))){
        correct=false;
        break;
      }
    }
    ...
    public static void verifyDoubleList(String output, String answer, DoubleListParser outputParser,
        ...
        assertEquals(answerList.length, outputList.length);
